...
bad.genes=unique(c(bad.genes,"HLA-DRB1","LY6G6D","LY6G6F-LY6G6D","UBE2L6",
                   "ASCL2","BCAS1","CDH11","FCGBP","HMG20B")) 
...
alpha=0.9 #manually change to the best value by generated figure above
...
# selected=read.table("FFPE.Selected.Genes.txt",header = T)
# selectedFF=read.table("FF.Selected.Genes.txt",header = T)
# setdiff(names(gene.freqs),selected$Var1)
# setdiff(names(gene.freqs),c(selected$Var1,selectedFF$Var1))
# setdiff(selected$Var1,names(gene.freqs))
# gene.freqs[selected$Var1]
# gene.freqs[setdiff(names(gene.freqs),c(selected$Var1,selectedFF$Var1))]

genes=names(gene.freqs)[gene.freqs>5]
new_exp=Combine_Normalize_FFPE(genes)
exprs=new_exp$exprs
exprs.nano=new_exp$exprs.nano
...
# setdiff(selected$Var1,names(gene.freqs))
# gene.freqs[selected$Var1]
# gene.freqs[setdiff(names(gene.freqs),c(selected$Var1))]
...
gene.freqs = gene.freqs[order(gene.freqs, decreasing=T)]
names(gene.freqs)=rownames(exprs)[as.numeric(names(gene.freqs))] 
write.table(gene.freqs, "FFPE.Selected.Genes.txt",row.names = F)
